...
main() -> lists:foreach(fun(Solution) -> io:fwrite("~s~n", [is_alice(Solution)]) end, get_test_cases(get_int_from_stdin())).
...
almost_sorted([X,Y,Z|T]) when X > Z -> is_sorted([X,Y|T]);
almost_sorted([X,Y,Z|T]) when Y > Z -> is_sorted([X,Z|T]);
almost_sorted([_,Y,Z|T]) -> almost_sorted([Y,Z|T]).

is_sorted([_]) -> true;
is_sorted([X,Y|_]) when X > Y -> false;
is_sorted([_,Y|T]) -> is_sorted([Y|T]).

win_test_case(TestCase) ->
  case almost_sorted(TestCase) of
  true -> true;
  false -> not lists:all(fun(Element) -> win_test_case(TestCase -- [Element]) end, TestCase)
  end.
